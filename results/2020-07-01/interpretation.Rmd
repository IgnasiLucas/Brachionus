...
On `2020-06-30` I repeated the enrichment analysis of gene ontology terms after
ordering the genes by *t* statistic (instead of *p* value), which facilitates
interpretation by separating genes underregulted in random environments from genes
upregulated in the random environment. Here, I want to do the volcano plots and
aid interpretation. See [@GarciaRoger2019] for some context.

I import results of the differential expression analysis (`2020-01-08`) to identify
the genes annotated with the significant GO terms, and to assess the direction of
gene expression change between selective regimes.
...
ENRICHMENT   <- '../2020-06-30/Enrichment.genes.regime.RData'
```
...
```{r data}
Annotation <- read.table(ANNOTATION, col.names = c('gene', 'GOterms'),
...
For adequate interpretation of some categories, I will need to check the original
functional annotation performed with interproscan on `2019-07-19`. From one of the
output tables, I select the most informative columns, potentially including names
of functional domains detected in transcripts. I also need the correspondance between
genes and transcripts, which is found in `2019-07-10/transcripts.fa`.
...
Several significant GO terms are related to signal transduction, such as:
signal transduction, G protein-coupled receptor signaling pathway, G protein-coupled
receptor activity, transmembrane signaling receptor activity, G protein-coupled peptide
receptor activity, G-protein beta/gamma-subunit complex binding, in addition to terms
related with transmembrane transport (see below).

```{r signalDensity, fig.width=12, fig.height=12}
goterms <- c('GO:0007186','GO:0007165',
             'GO:0004930','GO:0004888','GO:0005328','GO:0004983','GO:0008528','GO:0031683')
ontos   <- c('BP','BP','MF','MF','MF','MF','MF','MF')
p <- lapply(1:length(goterms), function(x) {
  showGroupDensity(eval(parse(text = paste0('GOdata.', ontos[x]))),
                   goterms[x], rm.one = FALSE)
})

grid.arrange(grobs=p, nrow=3)
```

```{r signalVolcano, fig.width=12, fig.height=12}
p <- lapply(goterms, function(x) volcanoPlot(term=x))
grid.arrange(grobs=p, nrow=3, 
             ...
             top = grid.text('Functions associated with signal transduction', gp = gpar(fontsize = 18)))
```
