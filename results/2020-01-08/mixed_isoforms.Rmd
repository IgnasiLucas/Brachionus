...
The `variancePartition` package [@Hoffman2016] fits mixed models to the expression data of every isoform in the
...
# Variance partition
When fitting the model to every isoform's expression profile, information from all isoforms contribute to
...
Curiously, though, a model needs to be fit first, for every isoform. The residual standard deviation and the
average isoform expression level of that fit is one point in the scatterplot where the mean-variance relationship
becomes apparent. When fitting these isoform-wise linear models, `voom()` does not consider random errors, but
...
# library is equal to `threshold`. Only isoforms expressed in at least 4 samples are retained.
...
Below I write the lists of isoforms with their fraction of expression variance explained by either selective
regime or hatching condition. The isoforms are ordered from the one the expression of which is most affected
by that variable to the one that is least affected. Note that among the top isoforms there will be both positive
and negatively affected isoforms.
...
list of p values. That is, it estimates the proportion of isoforms the expression of which is *not*
...
The `dream()` function uses a different, estimated number of degrees of freedom for every isoform, which
...
means the isoform is expressed more in the random than in the regular regime. And for the hatching condition
(or *treatment* coefficient), positive z-score (or log-fold change) means that the isoform is expressed more
under forced diapause (samples labelled with the *C* letter) than under immediate hatching.

I doubt whether to separate up- and downregulated isoforms in opposite ends of the isoform list that will be used
in functional enrichment analysis. It is clear that if a functional category contain isoforms both up- and downregulated,
I may fail to identify that category as significant if I separate them [@Ackermann2009]. For the moment, I will use the
p-value, rather than the z-score, to order isoforms, ignoring the sign of the effect. This will make the lists
