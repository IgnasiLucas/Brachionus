---
title: "Population-specific effects of randomness on isoform expression"
...
are interesting. Not only the isoforms that are *on average* differently expressed in regular than
in random environments are interesting. The specific response of each random population may have
biological meaning.

On `2019-04-03` we also determined that there are no isoforms with a significant interaction between
...
# Isoforms expressed *on average* differently between random and regular regimes
...
Instead of reporting the number of significantly regulated isoforms under a specific FDR,
I report here the overall, FDR-independent, estimated fraction of regulated
isoforms. There are ways to estimate the number of true null hypotheses even when
...
overestimates the proportion of non-regulated isoforms.
...
up to `r round(1.0 - min(randomness.pi0$pi0.est), 2) * 100`% of isoforms are differentially expressed
on average between regular and random regimes.

# Isoforms expressed differently in random population 1 than in the average regular population

Using again model 7, now I want to determine what isoforms are expressed differently in random
...
up to `r round(1.0 - min(random1.pi0$pi0.est), 2) * 100`% of isoforms are expressed differently
in population 1, under a random regime, relative to the average across populations in the
regular regime.

# Isoforms expressed differently in random population 2 than in the average regular population
...
up to `r round(1.0 - min(random2.pi0$pi0.est), 2) * 100`% of isoforms are expressed differently
in population 2, under a random regime, relative to the average across populations in the
regular regime.

# Isoforms expressed differently in (random) population 4 than in the average regular population

The third population under a random regime is labeled *4* in samples names. Let's see what and
how many isoforms are expressed differently in that population, relative to those under regular
regime:
...
up to `r round(1.0 - min(random3.pi0$pi0.est), 2) * 100`% of isoforms are expressed differently
in population 3, under a random regime, relative to the average across populations in the
regular regime.

# Isoforms always expressed differently in a random population than in the average regular population

The scatterplot of p-values is not expected to produce any visible correlation, unless for the smallest
p-values. Plotting p-values in logarithmic scale does show that man isoforms differentially expressed
...
What about the log-fold change? Does the random regime change isoform expression in the same direction in
all populations? Yes:
...
Given the large number of regulated isoforms, I can use a conservative FDR to compare the most significantly
regulated isoforms among populations in the random regime.
...
A large proportion of isoforms up- or downregulated in any population under the random regime is also regulated
in at least one other population under the random regime. The first and third populations in the random regime
are more similar among them. Population 2 being the one with more isoforms regulated by the selective regime is also
the one with the largest proportion of exclusively regulated isoforms.

I should save the results of the `glmQLFTest()` to run the functional analysis on the lists of regulated isoforms in
...
```{r saving}
save(randomness, random1, random2, random3, file = 'isoforms/RegimeEffect_model7.RData')
write.table(topTags(randomness, n=NumOfTags)[[1]],
            file = 'isoforms/randomness.txt',
            quote = FALSE)
write.table(topTags(random1, n=NumOfTags)[[1]],
            file = 'isoforms/random1.txt',
            quote = FALSE)
write.table(topTags(random2, n=NumOfTags)[[1]],
            file = 'isoforms/random2.txt',
            quote = FALSE)
write.table(topTags(random3, n=NumOfTags)[[1]],
            file = 'isoforms/random3.txt',
            quote = FALSE)
```
