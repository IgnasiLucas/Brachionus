...
be translated upon exit from dormancy.
...
the other regime. And the patter is actually not consistent: two genes from this pathway are quite
...
Among the `r dim(z)[1]` most significantly differentially expressed genes presumably
involved in the `r Term(GOTERM[goterm])`, the `r length(PosLFC)` that are overexpressed
in the unpredictable environment encode some protein domains that are
not found among the `r length(NegLFC)` genes overexpressed in the regular regime; namely:
`r paste(zRandom[!zRandom %in% zRegular], collapse=', ')`.

Likewise, among the genes involved in this biological process that are overexpressed in
the regular environment, we see the following protein domains encoded: 
`r paste(zRegular[!zRegular %in% zRandom], collapse=', ')`.
...
this category that are regulated by the selective regime.
...
Among the `r length(genes)` annotated with the term for `r Term(GOTERM[goterm])`, only
`r dim(z)[1]` are significantly regulated by the selective regime at 0.1 FDR:
`r length(PosLFC)` upregulated in the unpredictable regime, and
`r length(NegLFC)` upregulated in the regular regime. The protein domains encoded in
the former but not in the latter are: `r paste(zRandom[!(zRandom %in% zRegular)], collapse=', ')`.

And found only in the genes overexpressed in the regular environment, we have:
`r paste(zRegular[!(zRegular %in% zRandom)])`.
